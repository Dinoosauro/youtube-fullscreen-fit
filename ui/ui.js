...
for (let item in check) {
    ...
    (chrome ?? "") !== "" ? chrome.storage.sync.get([check[item]], (res) => { document.getElementById(item).checked = res[check[item]] === "1" || document.getElementById(item).getAttribute("data-defcheck") === "a" }) : chrome.storage.sync.get([check[item]]).then((res) => { document.getElementById(item).checked = res[check[item]] !== "0" }); // Get the value of that item and update the DOM
}
